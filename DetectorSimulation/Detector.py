...
class Module:
    ...
    def __init__(self, typ, label, x_index: float, y_index: float, z_index: float, x_dim: int, y_dim: int, z_dim: int, long_direction: int, detector):
        ...
        self.tracker_hits = []
        # also keep track of distance to module formed using the next values of x_mesh, y_mesh (helps with trigger) - only keep track for z-sensor planes, as only those used for trigger
        self.next_x = None 
        self.next_y = None
    ...
    def set_next_x(self, dist):
        self.next_x = dist
        
    def set_next_y(self, dist):
        self.next_y = dist
        ...
        if len(hits) >= min_hits:
            for i in range(len(hits)-min_hits+1):  # index by bottom layer of the set of min_hits layers
                if hits[i].hit_module.typ == 'z': # should always be true as only z-layer used
                    if abs(hits[i].hit_module.x_index-hits[i+min_hits-1].hit_module.x_index) <= 2*neighbourhood * np.round(hits[i].hit_module.next_x,3) and \
                            abs(hits[i].hit_module.y_index-hits[i+min_hits-1].hit_module.y_index) <= 2*neighbourhood * np.round(hits[i].hit_module.next_y,3):
                        value = True
                ...
                for i in range(len(self.module_groups[key]["x_mesh"])):
                    ...
                    for j in range(len(self.module_groups[key]["y_mesh"])):
                        ...
                        for k in range(len(self.module_groups[key]["z_layer"])):
                            ...
                            # get inter-module distance - to be used during trigger, so only keep track for z-sensor planes
                            # CAN TAKE DIFFERENT INTER-MODULE DISTANCE IN x AND y DIRECTIONS
                
                            # find distance to next module along x
                            ...
                            self.module_groups[key]["modules"][(x, y, z)].set_next_x(dist)
                            
                            # find distance to next module along y
                            if j != len(self.module_groups[key]["y_mesh"]) - 1:
                                dist = abs(self.module_groups[key]["y_mesh"][j+1] - self.module_groups[key]["y_mesh"][j])
                            else:
                                dist = abs(self.module_groups[key]["y_mesh"][j] - self.module_groups[key]["y_mesh"][j-1])
                            
                            self.module_groups[key]["modules"][(x, y, z)].set_next_y(dist)
